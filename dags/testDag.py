...
def update_skip_count(dag_id, **kwargs):
    ...
    try:
        ...
        logging.info(f'Current Skip Count: {skipped_tasks[0]}')
        ...
        for task_id, skip_count in skipped_tasks:
            ...
            update_query=f"""
                UPDATE public.skipped_data SET skip_count = %s , timestamp = %s WHERE dag_id=%s AND task_id = %s;
            """
            cursor.execute(update_query, (new_skip_count, datetime.now(), dag_id, task_id))

            logging.info(f'Updated skip count for task_id: {task_id}, new skip count = {new_skip_count}')


            if new_skip_count ==skips_allowed:
                logging.warning(f'dag_id: {dag_id} and task_id: {task_id} have been skipped {skips_allowed} times consecutively')
                with open('dags/error_log.txt', 'w') as file:
                    file.write(f'dag_id: {dag_id} and task_id: {task_id} have been skipped {skips_allowed} times consecutively')
